  ...
  <div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
    <div class="bg-white rounded-lg shadow-xl w-full max-w-2xl max-h-[80vh] flex flex-col">
      <div class="p-4 border-b border-gray-200 flex items-center justify-between">
        <h2 class="text-lg font-semibold text-gray-900">{{ editingAccount ? 'Edit Account' : 'Add Account' }}</h2>
        <button
          @click="$emit('close')"
      ...
      <div class="flex-1 overflow-y-auto p-4 space-y-4">
        ...
        <div v-if="form.type === 'pop3'">
          <label class="block text-sm font-medium text-gray-700 mb-1">POP3 Server</label>
          <div class="flex space-x-2">
            <input
...
const saveAccount = async () => {
  ...
  if (!editingAccount.value && form.value.authType === 'password' && !form.value.password) {
